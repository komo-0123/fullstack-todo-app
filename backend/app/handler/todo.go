...
func GetTodos(w http.ResponseWriter, _ *http.Request) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&todo.ID, &todo.Title, &todo.IsComplete); err != nil {
			response.WriteJSON(w, []model.Todo{}, http.StatusInternalServerError, constant.DB_ERR_FAILED_GET_TODO_ROW)
			return
		}
		todos = append(todos, todo)
	}

	response.WriteJSON(w, todos, http.StatusOK, "")
}
...
func CreateTodo(w http.ResponseWriter, r *http.Request) {
	...
	response.WriteJSON(w, []model.Todo{}, http.StatusCreated, "")
}
